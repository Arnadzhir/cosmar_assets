...
$sparepart_id = (int)$_GET['id'];

// PERBAIKAN: Ambil data sparepart
$query = "
    SELECT 
        s.sparepart_id,
        s.sparepart_qty,
    ...
    WHERE s.sparepart_id = $sparepart_id AND s.disposal_status = 2
";
...
$images = [];
if (!empty($sparepart['sparepart_image'])) {
    $images[] = $sparepart['sparepart_image'];
}
...
if (!empty($sparepart['disposal_date'])) {
    $disposal_timestamp = strtotime($sparepart['disposal_date']);
...
// Hitung umur
$umur = hitungUmur($sparepart['disposal_date'], $sparepart['asset_date']);
$total_nilai = ($sparepart['sparepart_price'] ?? 0) * ($sparepart['sparepart_qty'] ?? 0);

// Buat nomor BA
$ba_number = 'DISPOSAL/SP/' . date('Ymd') . '/' . str_pad($sparepart_id, 4, '0', STR_PAD_LEFT);
...
<head>
    ...
    <title>Berita Acara Disposal Sparepart - <?= htmlspecialchars($sparepart['sparepart_name']) ?></title>
...
<!-- HALAMAN 1: Berita Acara Disposal Sparepart -->
<div class="page">
    <!-- Header -->
    <div class="header">
        <img src="<?= BASE_URL ?>master/img/cosmar_logo.png" alt="Logo Perusahaan" class="logo">
        <h1>BERITA ACARA DISPOSAL SPAREPART</h1>
        <p style="font-size: 10px; text-align: center;">Nomor: <?= $ba_number ?></p>
    </div>

    <!-- Kalimat Pembuka -->
    <div class="content">
        <p>Pada hari <strong><?= $tanggalFormatted ?></strong>, telah dilakukan proses Disposal (Penghapusan) terhadap sparepart dengan rincian sebagai berikut:</p>
    </div>
    ...
    <div class="content">
        ...
        <table>
            <tr>
                <th>Kode Asset</th>
                <td><strong><?= htmlspecialchars($sparepart['assets_kode'] ?? '-') ?></strong></td>
            </tr>
            <tr>
                <th>Nama Asset</th>
                <td><?= htmlspecialchars($sparepart['assets_name'] ?? '-') ?></td>
            </tr>
            <?php if (!empty($sparepart['assets_model'])): ?>
            <tr>
                <th>Model Asset</th>
                <td><?= htmlspecialchars($sparepart['assets_model']) ?></td>
            </tr>
            <?php endif; ?>
            <tr>
                <th>Kategori</th>
                <td><?= !empty($sparepart['kategori_name']) ? htmlspecialchars($sparepart['kategori_name'] . ' - ' . $sparepart['kategori_line']) : '-' ?></td>
            </tr>
            <tr>
                <th>Type</th>
                <td><?= htmlspecialchars($sparepart['type_name'] ?? '-') ?></td>
            </tr>
            <tr>
                <th>Merk Asset</th>
                <td><?= htmlspecialchars($sparepart['merk_name'] ?? '-') ?></td>
            </tr>
            <tr>
                <th>Spesifikasi Asset</th>
                <td><?= !empty($sparepart['asset_spec']) ? nl2br(htmlspecialchars($sparepart['asset_spec'])) : '-' ?></td>
            </tr>
            <tr>
                <th>Tanggal Beli Asset</th>
                <td><?= !empty($sparepart['asset_date']) && $sparepart['asset_date'] != '0000-00-00' ? date('d/m/Y', strtotime($sparepart['asset_date'])) : '-' ?></td>
            </tr>
            <tr>
                <th>Harga Asset</th>
                <td>Rp <?= number_format($sparepart['asset_price'] ?? 0, 0, ',', '.') ?></td>
            </tr>
            <tr>
                <th>Estimasi Umur Asset</th>
                <td><?= htmlspecialchars($sparepart['assets_life'] ?? '-') ?> Tahun</td>
            </tr>                                    
        </table>
    </div>
    ...
    <!-- 2. DATA SPAREPART -->
    <div class="content">
        <h2>2. DATA SPAREPART</h2>
        <table>
            <tr>
                <th>Nama Sparepart</th>
                <td><?= htmlspecialchars($sparepart['sparepart_name'] ?? '-') ?></td>
            </tr>
            <tr>
                <th>Merk Sparepart</th>
                <td><?= htmlspecialchars($sparepart['sparepart_merk'] ?? '-') ?></td>
            </tr>
            <tr>
                <th>Spesifikasi</th>
                <td><?= !empty($sparepart['sparepart_spec']) ? nl2br(htmlspecialchars($sparepart['sparepart_spec'])) : '-' ?></td>
            </tr>
            <tr>
                <th>Quantity</th>
                <td><?= number_format($sparepart['sparepart_qty'] ?? 0) ?> unit</td>
            </tr>
            <tr>
                <th>Harga Sparepart</th>
                <td>Rp <?= number_format($sparepart['sparepart_price'] ?? 0, 0, ',', '.') ?></td>
            </tr>
            <tr>
                <th>Total Nilai</th>
    ...
    <div class="content">
        ...
        <table>
            <tr>
                ...
                <td><?= nl2br(htmlspecialchars($sparepart['disposal_reason'] ?? '-')) ?></td>
            </tr>
            <tr>
                <th>Tanggal Pengajuan</th>
                <td><?= !empty($sparepart['disposal_date']) ? date('d/m/Y H:i', strtotime($sparepart['disposal_date'])) : '-' ?></td>
            </tr>
        </table>
    </div>

    <!-- 4. PIHAK TERKAIT (menggunakan tbl_karyawan) -->
    <div class="content">
        <h2>4. PIHAK TERKAIT</h2>
        <table>
            <tr>
                <th>Penanggung Jawab / User</th>
                <td><strong><?= htmlspecialchars($sparepart['karyawan_name'] ?? '-') ?></strong></td>
            </tr>
            <tr>
                <th>ID Karyawan</th>
                <td><?= htmlspecialchars($sparepart['karyawan_no'] ?? '-') ?></td>
            </tr>
            <tr>
                <th>Departemen</th>
                <td><strong><?= htmlspecialchars($sparepart['dep_name'] ?? '-') ?> (<?= htmlspecialchars($sparepart['dep_code'] ?? '-') ?>)</strong></td>
            </tr>
        </table>
    </div>

    <!-- Informasi Tambahan: Umur Sparepart -->
    <?php if ($umur != '-'): ?>
    <div class="content">
        <h2>5. INFORMASI TAMBAHAN</h2>
        <table>
            <tr>
                <th>Umur Sparepart saat Disposal</th>
                <td><?= $umur ?></td>
            </tr>
        </table>
    </div>
    ...
    <div class="signature">
        ...
        <div class="signature-right">
            ...
            <div class="signature-line"></div>
            <p style="text-align: center;"><strong><?= htmlspecialchars($sparepart['karyawan_name'] ?? '-') ?></strong></p>
            <p style="text-align: center;"><?= htmlspecialchars($sparepart['dep_name'] ?? '-') ?></p>
...
<div class="page page-break">
    <div class="header">
        ...
        <p style="font-size: 10px;">Berita Acara Disposal Sparepart - <?= htmlspecialchars($sparepart['sparepart_name']) ?></p>
    </div>
    ...
    <div class="gallery">
        ...
        <div class="gallery-item">
            <img src="<?= BASE_URL ?>master/img/assets/<?= htmlspecialchars($img) ?>" alt="Gambar Sparepart <?= $no ?>" 
    ...
    <div class="footer-note">
        <p><em>Halaman ini merupakan lampiran dari Berita Acara Disposal Sparepart.</em></p>
    </div>
</div>
<?php endif; ?>
