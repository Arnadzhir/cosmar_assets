...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-tags"></i> Laporan Merk Asset
        </h1>
    </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select name="merk_id" id="merk_id" class="form-control select2">
                                ...
                                <?php
                                $qMerk = mysqli_query($conn, "
                                    SELECT merk_id, merk_name 
                                    FROM tbl_merk 
                                    ORDER BY merk_name
                                ");
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="d-flex">
                                ...
                                <a href="merk.php" class="btn btn-reset">
    ...
    <!-- CARD 2: DAFTAR ASSETS BERDASARKAN MERK -->
        ...
        <div class="card-body">
            ...
            if ($show_data) {
                ...
                $query = "
                    ...
                    INNER JOIN tbl_merk m ON a.merk_id = m.merk_id
                ...
                $filter_nama = '';
                if (isset($_GET['merk_id']) && !empty($_GET['merk_id'])) {
                    $filter_merk = mysqli_real_escape_string($conn, $_GET['merk_id']);
                    $query .= " AND m.merk_id = '$filter_merk'";
                    
                    $qNamaMerk = mysqli_query($conn, "SELECT merk_name FROM tbl_merk WHERE merk_id = '$filter_merk'");
                    $namaMerk = mysqli_fetch_assoc($qNamaMerk);
                    $filter_nama = $namaMerk['merk_name'] ?? '';
                }
                ...
                $query .= " ORDER BY m.merk_name ASC, a.assets_name ASC, p.primary_id ASC";
                ...
                <div class="info-header">
                    <div class="filter-info">
                        <?php if (isset($_GET['merk_id']) && !empty($_GET['merk_id'])): ?>
                            <i class="fas fa-filter"></i> Menampilkan asset dengan merk: <strong><?= htmlspecialchars($filter_nama) ?></strong>
                        <?php else: ?>
                        ...
                        <table class="table table-bordered" id="dataTable">
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_assoc($qAssets)): 
                                    ...
                                    if (!isset($_GET['merk_id']) && $current_merk != $row['merk_name']) {
                                        if ($current_merk != '') {
                                            ?>
                                            <tr class="total-row">
                                                <td colspan="12" class="font-weight-bold text-right pr-4">
                                                    Total <?= $current_merk ?>: <?= $merk_total ?> unit
                                                 </div>
                                              </div>
                                            <?php
                                        }
                                        
                                        $current_merk = $row['merk_name'];
                                        $merk_total = 0;
                                        ?>
                                        <tr class="merk-group">
                                ...
                                <tr>
                                    ...
                                    <td><?= $asset_name ?> </div>
                                    <td><strong><?= htmlspecialchars($row['merk_name']) ?></strong> </div>
                                ...
                                <?php if (!isset($_GET['merk_id']) && $current_merk != ''): ?>
                                <tr class="total-row">
                                    <td colspan="12" class="font-weight-bold text-right pr-4">
                                        Total <?= $current_merk ?>: <?= $merk_total ?> unit
                                     </div>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Tidak ada asset yang ditemukan untuk merk yang dipilih.</p>
                    </div>
                <?php endif; ?>
            ...
            ?>
                <div class="empty-state">
                    ...
                    <p>Silakan pilih merk terlebih dahulu.</p>
                </div>
            <?php } ?>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    <?php if (isset($_GET['merk_id']) && !empty($_GET['merk_id'])): ?>
    $('#merk_id').val('<?= $_GET['merk_id'] ?>').trigger('change');
    <?php endif; ?>
});
</script>
