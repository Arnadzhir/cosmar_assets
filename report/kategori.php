...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-list"></i> Laporan Kategori Asset
        </h1>
    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="kategori_id" id="kategori_id" class="form-control select2">
                                ...
                                while ($kategori = mysqli_fetch_assoc($qKategori)) {
                                    echo "<option value='{$kategori['kategori_id']}'>{$kategori['kategori_name']} - {$kategori['kategori_line']}</option>";
                                }
                                ?>
                            </select>
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <div class="d-flex">
                                ...
                                <a href="kategori.php" class="btn btn-reset">
    ...
    <!-- CARD 2: DAFTAR ASSETS BERDASARKAN KATEGORI -->
        ...
        <div class="card-body">
            ...
            if ($show_data) {
                ...
                $query = "
                    ...
                    INNER JOIN tbl_kategori kat ON a.kategori_id = kat.kategori_id
                ...
                $filter_nama = '';
                if (isset($_GET['kategori_id']) && !empty($_GET['kategori_id'])) {
                    $filter_kategori = mysqli_real_escape_string($conn, $_GET['kategori_id']);
                    $query .= " AND kat.kategori_id = '$filter_kategori'";
                    
                    $qNamaKategori = mysqli_query($conn, "SELECT kategori_name, kategori_line FROM tbl_kategori WHERE kategori_id = '$filter_kategori'");
                    $namaKategori = mysqli_fetch_assoc($qNamaKategori);
                    $filter_nama = $namaKategori['kategori_name'] . ' - ' . $namaKategori['kategori_line'];
                }
                ...
                $query .= " ORDER BY kat.kategori_name ASC, kat.kategori_line ASC, a.assets_name ASC, p.primary_id ASC";
                ...
                <div class="info-header">
                    <div class="filter-info">
                        <?php if (isset($_GET['kategori_id']) && !empty($_GET['kategori_id'])): ?>
                            <i class="fas fa-filter"></i> Menampilkan asset dengan kategori: <strong><?= htmlspecialchars($filter_nama) ?></strong>
                        <?php else: ?>
                        ...
                        <table class="table table-bordered" id="dataTable">
                            ...
                            <tbody>
                                ...
                                while ($row = mysqli_fetch_assoc($qAssets)): 
                                    ...
                                    $kategori_display = $row['kategori_name'] . ' - ' . $row['kategori_line'];
                                    
                                    if (!isset($_GET['kategori_id']) && $current_kategori != $kategori_display) {
                                        if ($current_kategori != '') {
                                            ?>
                                            <tr class="total-row">
                                                <td colspan="12" class="font-weight-bold text-right pr-4">
                                                    Total <?= $current_kategori ?>: <?= $kategori_total ?> unit
                                                 </div>
                                             </td>
                                            <?php
                                        }
                                        
                                        $current_kategori = $kategori_display;
                                        $kategori_total = 0;
                                        ?>
                                        <tr class="kategori-group">
                                ...
                                <tr>
                                    ...
                                    <td><?= $asset_name ?> </div>
                                    <td><strong><?= $kategori_display ?></strong> </div>
                                ...
                                <?php if (!isset($_GET['kategori_id']) && $current_kategori != ''): ?>
                                <tr class="total-row">
                                    <td colspan="12" class="font-weight-bold text-right pr-4">
                                        Total <?= $current_kategori ?>: <?= $kategori_total ?> unit
                                     </div>
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Tidak ada asset yang ditemukan untuk kategori yang dipilih.</p>
                    </div>
                <?php endif; ?>
            ...
            ?>
                <div class="empty-state">
                    ...
                    <p>Silakan pilih kategori terlebih dahulu.</p>
                </div>
            <?php } ?>
        </div>
    </div>
...
$(document).ready(function() {
    ...
    <?php if (isset($_GET['kategori_id']) && !empty($_GET['kategori_id'])): ?>
    $('#kategori_id').val('<?= $_GET['kategori_id'] ?>').trigger('change');
    <?php endif; ?>
});
</script>
